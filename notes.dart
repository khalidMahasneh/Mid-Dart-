...
print(mylist.sublist(1)); //[2,3,4,5] // if end not provided it returens all element from statr to the lenght 
print(mylist.shuffel());
mylist.sort();
...
//map() method we can use map iterator to multiple each element in the list with number 2 using Curly brackets
heights.map((e) {
return e * 2;
});
 // we can use map iterator to multiple each element in the list with number 2 using Fat arrow  
 heights.map((e) => e*5,); // The expression after => is implicitly returned (no need for return).

// However, this command do not modify the list
// Therefore, we need to assign it to height or create new variable
var newHeights = heights.map((e) {
return e * 2;
});
print(newHeights); //Note that newHeights is not a list
//Therefore, we need to convert it to list
var newHeightsList = heights
.map((e) {
return e * 2;
}).toList(growable: true);
print(newHeightsList); // this is list 

//uses the map() method to return an iterable of numbers (double) from a list of strings
var inputs = ['1.24', '2.35', '4.56', 'abc'];
var numbers = inputs.map((n) => double.tryParse(n)); 
print(numbers); //(1.24, 2.35, 4.56, null)

//Since the double.tryParse() method is the same as the function argument of the map() method
//you can directly pass the double.tryParse method to the map() method instead of using an anonymous function:
var inputs = ['1.24', '2.35', '4.56', 'abc'];
...
print(weekday); // print("Value of weekday is $weekday");
